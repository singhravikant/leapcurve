...
;(function ($, document, window){
	...
	var op_id = $('.active').attr('id');
	if($("#page_url").val()=='page1') {
	var opcheckL = $("input[name='left"+op_id+"']:checked").val();
	var opcheckR = $("input[name='right"+op_id+"']:checked").val();
	var comThree = $("input[name='left10']:checked").val();
	//alert(comThree);
	var output = true;
	if(opcheckL && opcheckR){
		//alert("-----");
		$("#exname-error"+op_id).html("");
	}else if(comThree && op_id==27){
		//alert("---rrr--");
		$("#exname-error"+op_id).html("");
	}else if(op_id==26||op_id==25||op_id==5||op_id==3){
		$("#exname-error"+op_id).html("");		
	}else{
	var output = false;
	//alert("--eeee---");
	$("#exname-error"+op_id).html("Please select your options.");	
		
	}
   
	}else if($("#page_url").val()=='page2'){
	var op_id = $('.active').attr('id');
	var opcheckL = $("input[name='left"+op_id+"']:checked").val();
	var comThree = $("input[name='left10']:checked").val();
	//alert(op_id);
	...
	if(output==true){
		...
		that.$element.find('.finished-text').on('click', function(e){
			var op_id = $('.active').attr('id');
	if($("#page_url").val()=='page1') {
	output = true;
	if(!($("#Lusersalery").val()) || !($("#Rusersalery").val())) {
		output = false;
			$("#sexname-error").html("Please enter your current salary.");
	}else{
		$("#sexname-error").html("");
	}
	...
	methods = {
		init: function (element, options) {
			...
			if(opts.bottomButtons && !bottomActions.length){
				...
				html += '<span class="btn btn-default btn-mini btn-xs btn-prev"><span class="previous-text">'+ opts.text.previous +'</span></span>';
				html += '<span class="btn btn-default btn-mini btn-xs btn-next"><span id="next-val" class="next-text">'+ opts.text.next +'</span><span class="finished-text"><input type="submit" name="submit" class="steps" value="'+ opts.text.finished +'"></span></span>';
				html += '</div>';
